...
8. BOSH emits _cannot convert string to integer_? Look **very** carefully at deployment manifest. Doesn't work yet? Ok, _ssh_ to BOSH machine, look into logs over there (in `/var/vcap/sys/log`). No clue? I'm still with you: leave only one _resque_ worker alive: `monit summary`, `monit status`, `monit stop worker_2`, `monit stop worker_3` are your friends. Start adding logging to BOSH Ruby gems somewhere under `/var/vcap` (at least everything is under that legacy-named directory). Then `monit restart worker_1`. Resubmit the deployment. Wash-repeat.
